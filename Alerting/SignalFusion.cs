...
namespace HoneytokenWatcher.Alerting
{
    ...
    public class SignalFusion : IDisposable
    {
        ...
        /// <summary>
        /// Returns a snapshot of all processes with live (non-decayed) signals,
        /// sorted by fused score descending. Used by the dashboard for live display.
        /// </summary>
        public List<ProcessScoreSnapshot> GetCurrentScores()
        {
            var cutoff = DateTime.Now.AddMinutes(-SignalDecayMinutes);
            var result = new List<ProcessScoreSnapshot>();

            lock (_lock)
            {
                foreach (var kvp in _signals)
                {
                    var live = kvp.Value.Where(s => s.Timestamp >= cutoff).ToList();
                    if (live.Count == 0) continue;

                    // Best raw score per source (same logic as Evaluate)
                    var bestPerSource = new Dictionary<SignalSource, ThreatSignal>();
                    foreach (var sig in live)
                    {
                        if (!bestPerSource.TryGetValue(sig.Source, out var existing) ||
                            sig.RawScore > existing.RawScore)
                            bestPerSource[sig.Source] = sig;
                    ...
                    var latest = live.OrderByDescending(s => s.Timestamp).First();
                    result.Add(new ProcessScoreSnapshot
                    {
                        ProcessId   = kvp.Key,
                        ProcessName = latest.ProcessName,
                        ProcessPath = latest.ProcessPath,
                        FusedScore  = Math.Min((int)Math.Round(weightedSum * 100), 100),
                        SignalCount = live.Count,
                        Sources     = bestPerSource.Keys.ToList(),
                        BestSignals = new List<ThreatSignal>(bestPerSource.Values),
                        LastSeen    = latest.Timestamp,
                    });
                }
            }

            return result.OrderByDescending(p => p.FusedScore).ToList();
        }
    ...
    /// <summary>
    /// Snapshot of a single process's current threat state — used by the dashboard.
    /// </summary>
    public class ProcessScoreSnapshot
    {
        ...
        public List<SignalSource>  Sources     { get; set; } = new();
        public List<ThreatSignal>  BestSignals { get; set; } = new();
